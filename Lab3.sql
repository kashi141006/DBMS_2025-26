...
WHERE AMOUNT>4000 AND ACTO<105;
...
SELECT ACTO,CNAME,AMOUNT
FROM DEPOSIT
WHERE BNAME IN('AJNI','KAROLBAGH','M.G.ROAD') AND ACTO<104;
...
--1. Display all the detail of customer who deposited more than 5000 without using * from deposit table. 
    SELECT ACTO,CNAME,BNAME,AMOUNT,ADATE
    FROM DEPOSIT
    WHERE AMOUNT>5000
...
SELECT *
FROM DEPOSIT
WHERE (AMOUNT>2000) AND (ACTO BETWEEN 100 AND 110 ) AND (ADATE>'1995-3-1'AND ADATE<'1996-3-27');
